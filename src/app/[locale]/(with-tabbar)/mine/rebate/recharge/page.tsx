...
import {receiveWagesCz, wagesCzRecords} from "@/api/customer";
import {toast} from "sonner";
import {WagesCzRecordField} from "@/types/customer.type";
...
export default function RechargePage() {
  ...
  const [list, setList] = useState<WagesCzRecordField[]>([]);
  ...
  const fetchData = async (pageNo: number) => {
    setLoading(true);
    // 请求接口
    const {code, data, message} = await wagesCzRecords({
      pagination: {
        page: pageNo,
        size: 20,
      }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr_1fr_1fr] px-3 py-2 text-xs text-muted-foreground border-b">
        <div className="text-center">{_t('rebate.recharge-table-header-1')}</div>
        <div className="text-center">{_t('rebate.recharge-table-header-2')}</div>
        <div className="text-center">{_t('rebate.recharge-table-header-3')}</div>
        <div className="text-center">{_t('rebate.recharge-table-header-4')}</div>
        <div className="text-center">{_t('rebate.recharge-table-header-5')}</div>
      </div>
      {list.map((item, index) => (
        <div
          key={`all-key-${index}`}
        ...
        >
          ...
          {/* 有效流水 */}
          <div className="text-center font-medium">{format.number(item?.wpoints || 0)}</div>
