...
import {bindEmail, currentCustomer, getProfile, updateNickname} from "@/api/customer";
...
const schema = z.object({
  email: z.string().min(1, "请输入邮箱").max(50).regex(/^[\w\-\.]+@[\w\-]+(\.\w+)+$/, "邮箱格式不正确"),
});
    ...
    <>
      <div className="flex min-h-screen justify-center bg-[#eef3f8]">
        ...
        <div className="w-full max-w-xl bg-[#f5f7fb] shadow-sm">
          <PageHeader title="绑定邮箱" />
          {/* 提示 */}
          <main className="px-3 pb-20 pt-3">
            <Alert variant="destructive">
              <div className="flex items-center gap-2">
                <AlertCircleIcon />
                <AlertTitle>绑定后不可修改，请认真填写</AlertTitle>
              </div>
            </Alert>
            ...
            <form onSubmit={onSubmit} className="mt-5">
              <div className="bg-white rounded-xl shadow-sm p-2">
                ...
                <div className="flex justify-center items-center ">
                  <label className="w-1/7 text-gray-700">邮箱</label>
                  <input
                    type="text"
                    placeholder="请输入邮箱"
                    {...register("email")}
