...
import type { SignRecordField } from '@/types/customer.type'

const list = ref<SignRecordField[]>([])
const page = ref(1)
...
<template>
  ...
  <div class="grid grid-cols-[1fr_1fr_1fr] px-3 py-2 text-xs text-gray-500 border-b bg-white">
    ...
    <div class="text-center">获得积分</div>
    <div class="text-center">获得金豆</div>
  </div>
  ...
  >
    <div class="text-center">{{ item?.sign_date || '' }}</div>
    <div class="text-center font-medium text-red-500">+{{ item?.reward_points || 0 }}</div>
    <div class="text-center font-medium text-orange-500">+{{ item?.reward_base_coin || 0 }}</div>
  </div>
