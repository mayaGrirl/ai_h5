...
const currentValue = ref('safeQuestion')
...
const handleSubmit = async () => {
  ...
  try {
    const res = await settingViewCardVerifyType({
      select_card_verify_type: currentValue.value
    })
    ...
    if (res.code === 200) {
      ...
      if (authStore.currentCustomer) {
        authStore.setCurrentCustomer({
          ...
          selectcardVerifyType: currentValue.value
        })
      }
      router.back()
    } else {
...
// 初始化
onMounted(() => {
  if (authStore.currentCustomer?.selectcardVerifyType) {
    const radioValues = radioOptions.map(item => item.value)
    if (radioValues.includes(authStore.currentCustomer.selectcardVerifyType)) {
      currentValue.value = authStore.currentCustomer.selectcardVerifyType
    }
  }
})
</script>
...
<template>
  <div class="flex min-h-screen justify-center bg-[#eef3f8]">
    <div class="w-full max-w-xl bg-[#f5f7fb] shadow-sm">
      <PageHeader :title="t('mine.security-settings.group-other.card')" />
