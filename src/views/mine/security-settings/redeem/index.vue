...
const handleSubmit = async () => {
  ...
  try {
    const res = await settingRedeemGiftVerifyType({
      prize_verify_type: currentValue.value
    })
    ...
    if (res.code === 200) {
      ...
      if (authStore.currentCustomer) {
        authStore.setCurrentCustomer({
          ...
          prizeVerifyType: currentValue.value
        })
      }
      router.back()
    } else {
...
// 初始化
onMounted(() => {
  if (authStore.currentCustomer?.prizeVerifyType) {
    currentValue.value = authStore.currentCustomer.prizeVerifyType
  }
})
</script>
...
<template>
  <div class="flex min-h-screen justify-center bg-[#eef3f8]">
    <div class="w-full max-w-xl bg-[#f5f7fb] shadow-sm">
      <PageHeader :title="t('mine.security-settings.group-other.redeem')" />
