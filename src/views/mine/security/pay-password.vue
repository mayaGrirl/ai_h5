...
const validate = () => {
  ...
  if (!formData.value.password || formData.value.password.length < 8) {
    errors.value.password = t('mine.security-settings.group-account.pay-password.password-min')
    return false
  }
...
onMounted(async () => {
  try {
    const res = await getBlockByIdentifier('customer_pay_password_tips')
...
<template>
  <div class="flex min-h-screen justify-center bg-[#eef3f8]">
    <div class="w-full max-w-xl bg-[#f5f7fb] shadow-sm">
      <PageHeader :title="t('mine.setting.edit-pay-password')" />
        ...
        <form @submit.prevent="handleSubmit" class="w-full bg-gray-100 px-3 py-4">
          ...
          <div class="bg-white rounded-lg mb-3 overflow-hidden">
            <div class="flex items-center text-gray-600 px-4 py-3 pb-0">
              ...
              {{ t('mine.security-settings.group-account.pay-password.password') }}
            </div>
            <div class="flex items-center px-4 py-3 border-b">
              <label class="w-2/7 text-gray-700">{{ t('mine.security-settings.group-account.pay-password.password') }}</label>
              <div class="w-5/7 relative">
                <input
                  v-model="formData.password"
                  :type="showPassword ? 'text' : 'password'"
                  :placeholder="t('mine.security-settings.group-account.pay-password.password-placeholder')"
            ...
            <div class="flex items-center px-4 py-3">
              <label class="w-2/7 text-gray-700">{{ t('mine.security-settings.group-account.pay-password.confirm-password') }}</label>
              <div class="w-5/7 relative">
                <input
                  v-model="formData.confirm_password"
                  :type="showPassword ? 'text' : 'password'"
                  :placeholder="t('mine.security-settings.group-account.pay-password.confirm-password-placeholder')"
                  class="text-gray-600 placeholder-gray-400 focus:outline-none h-10 w-full pr-10"
                />
                <button
                  type="button"
