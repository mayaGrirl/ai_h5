...
<template>
  ...
  >
    ...
    <!-- 账变前 -->
    <div class="text-center font-medium text-gray-700">{{ (item?.b_points || 0).toLocaleString() }}</div>

    <!-- 账变 -->
    <div class="text-center font-medium" :class="(item?.a_points || 0) >= (item?.b_points || 0) ? 'text-green-500' : 'text-red-500'">
      {{ (item?.a_points || 0) >= (item?.b_points || 0) ? '+' : '-' }}{{ Math.abs(item?.points || 0).toLocaleString() }}
    </div>

    <!-- 账变后 -->
    <div class="text-center font-medium text-red-500">{{ (item?.a_points || 0).toLocaleString() }}</div>
